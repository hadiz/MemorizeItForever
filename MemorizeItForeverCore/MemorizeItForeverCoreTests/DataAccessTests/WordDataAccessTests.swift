...
class WordDataAccessTests: XCTestCase {
    ...
    override func setUp() {
        ...
        let dataAccess = MockGenericDataAccess<WordEntity>(context: context)
        let setDataAccess = MockGenericDataAccess<SetEntity>(context: context)
        wordDataAccess = WordDataAccess(genericDataAccess: dataAccess, setDataAccess: setDataAccess)
    }
    ...
    func testFetchWord(){
        do{
            ...
            wordModel.setId = NewSetEntity(context)?.setId
            try wordDataAccess.save(wordModel)
            let words = try wordDataAccess.fetchAll(fetchLimit: nil)
    ...
    private func NewSetEntity(_ initialContext: ManagedObjectContextProtocol) -> SetModel?{
        return TestFlowHelper().NewSetModel(initialContext: initialContext)
    }
